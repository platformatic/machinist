...
test('get services by metadata labels', async t => {
  ...
  const result = await app.inject({
    method: 'GET',
    url: '/k8s/services/default?labels=app.kubernetes.io/name%3Dmyapp&labels=plt.dev/version%3D1.2.4',
    headers: {
  ...
  const svc = services.find(s => s.name === 'myapp-v1-2-4')
  assert(svc)
  assert.strictEqual(svc.labels['app.kubernetes.io/name'], 'myapp')
  assert.strictEqual(svc.labels['plt.dev/version'], '1.2.4')
  assert(svc.ports.length > 0)
  assert(svc.ports[0].port)
})
...
test('returns empty array when no services match labels', async t => {
  ...
  const result = await app.inject({
    method: 'GET',
    url: '/k8s/services/default?labels=app.kubernetes.io/name%3Dnonexistent',
    headers: {
      'content-type': 'application/json'
    }
  })
